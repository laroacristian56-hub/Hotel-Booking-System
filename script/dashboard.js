...
document.addEventListener("DOMContentLoaded", () => { 
    ...
    const lastPage = localStorage.getItem('activeAdminPage');

    setTimeout(() => {
        if (lastPage === 'rooms') {
            openRoomPage();
        } else if (lastPage === 'bookings') {
            openBookingPage();
        } else if (lastPage === 'payment') {
            openPaymentPage();
        } else if (lastPage === 'reports') {
            openReportsPage();
        } else {
            openDashboardPage(); // Default
        }
    }, 500);
...
function closeAllPages() {
    ...
    pages.forEach(id => {
        ...
        if (page) {
            ...
            // Immediate hide to prevent layout shifts during refresh
            // But keep the fade effect if switching normally
...
function openDashboardPage() {
    saveState('dashboard'); // Save Memory
    closeAllPages();
    ...
    setTimeout(() => { pg.style.opacity = "1"; }, 50);
}

function openRoomPage() {
    saveState('rooms'); // Save Memory
    closeAllPages();
    mainTitle.innerText = "Browse Rooms";
...
function openBookingPage() {
    saveState('bookings'); // Save Memory
    closeAllPages();
...
function confirmLogout() {
    if(confirm("Are you sure you want to log out?")) {
        saveState('dashboard'); // Reset to dashboard on next login
